...
func TestClone(t *testing.T) {
	{
		u1, err := Parse("http://clipperhouse.com/foo?bar=qux")
		if err != nil {
			t.Fatal(err)
		}
		...
		if u1 == u2 {
			t.Error("should be different URL references")
		}
	}

	{
		u1, err := Parse("http://user:password@clipperhouse.com/foo?bar=qux")
		if err != nil {
			t.Fatal(err)
		}
		...
		if u1.user == u2.user {
			t.Error("should be different Userinfo references")
		}
	}
}
