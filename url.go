...
//
// This package is a fork of net/url, with tools for keeping URLs
// immutable as they are passed around. Accessing Host, Path, etc
// is done by calling methods instead of fields.
//
// To mutate a URL's query, call url.Query(), mutate those values,
// and then call url.SetQuery() using the result. This will return
// a new immutable URL. Similarly with SetPath().
package url
...
// SetQuery returns a new immutable URL with the query updated to q
func (u *URL) SetQuery(q Values) *URL {
	clone := u.clone()
...
// SetPath returns a new immutable URL with the path updated to path
...
// NewUser returns a Userinfo containing the provided username
// and no password set.
func NewUser(username string) *Userinfo {
